...
static gboolean queued_ip_config_change (gpointer user_data);
              ...
              const GValue *value, GParamSpec *pspec)
{
	...
	case PROP_IFACE:
		if (g_value_get_string (value)) {
			...
			ifindex = nm_platform_link_get_ifindex (priv->iface);
			if (ifindex > 0) {
			...
			} else if (!device_has_capability (self, NM_DEVICE_CAP_IS_NON_KERNEL))
				_LOGW (LOGD_HW, "failed to look up interface index");
		}
		break;
	case PROP_DRIVER:
