...
system_create_virtual_device (NMManager *self, NMConnection *connection, GError **error)
{
	...
	if (!nm_platform_link_exists (iface))
	...
	for (iter = connections; iter; iter = g_slist_next (iter)) {
		...
		if (!nm_device_create_and_realize (device, connection, parent, &local_err)) {
			nm_log_warn (LOGD_DEVICE, "(%s:%s): %s",
			             nm_connection_get_id (connection),
...
add_device (NMManager *self, NMDevice *device)
{
	...
	gboolean enabled = FALSE, platform_unmanaged;
	RfKillType rtype;
                     ...
                     NMPlatformReason reason)
{
	...
	if (factory) {
		...
		/* Ignore creation failures; some factories may want to suppress
		 * creation of devices for some link types, like WWAN_ETHERNET.
		 */
	} else {
		switch (plink->type) {
